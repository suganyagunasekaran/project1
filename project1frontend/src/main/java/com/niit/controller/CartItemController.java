...
private ProductDao productDao;
 ...
 /*   @RequestMapping(value="/cart/updatecartitem/{cartItemId}")
    public String updateCartItem(@RequestParam int requestedQuantity,Model model)
    { 
    	    CartItem cartItem=new CartItem();
            cartItem.setQuantity(requestedQuantity);
...
@RequestMapping(value="/cart/thanks")
public String showThanksPage(Model model){
	
	return "thanks";
 }

@RequestMapping(value="/cart/invoice")
public String showInvoicePage(@ModelAttribute ShippingAddress shippingaddress,
		Model model,
	@AuthenticationPrincipal Principal principal,HttpSession session){
    	String email=principal.getName();
    	User user=cartItemDao.getUser(email);
